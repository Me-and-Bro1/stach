...
typedef struct {
  ...
  uint64_t columns; // -c [cols] Specifies the number of octets (bytes) to display per line (default is 16).
  uint64_t group_bytes; // -g [bytes] Sets how many bytes are grouped together (default is 2).
  uint64_t length; // -l [len] Limits the output to a specific number of bytes.
  uint64_t seek; // -s [offset] Starts the dump at a specific byte offset in the file.
  bool autoskip; // -a Replaces duplicate or null lines with an asterisk (*) to shorten output.
  bool binary_bump; // -b Displays the output in binary bits (0s and 1s) instead of hex.
  bool plain; // -p Outputs a continuous hex string without line numbers or ASCII characters.
...
ProgramArgs init_args(const int argc, const char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg[0] != '-') {
      if (!args.input_file) args.input_file = arg;
      else if (!args.output_file) args.output_file = arg;
      else {
